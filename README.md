...
챗봇은 정보수집에 있어 목적이 표면에 드러나지 않기때문에 심리적 장벽이 낮다는 장점이 있습니다. 사용자와의 대화를 통해 성별, 연령, 심리 상태 등을 자연스럽게 파악해 이를 토대로 적절한 제품 추천이 가능합니다. 그 중에서도 저희 챗봇은 사용자와 나눈 대화를 분류 할 수 있다는 것이 가장 큰 특징 입니다. 6가지 감정 대분류를 사용자의 니즈로 가정하고, 기분에 맞는 에센셜 오일을 추천 해 주는 시스템을 구상했습니다.      
